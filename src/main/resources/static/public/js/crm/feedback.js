var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			...
			var url = vm.feedback.pid == null ? "../crm/feedback/save" : "../crm/feedback/update";
			$.ajax({
				type: "POST",
			    url: url,
		...
		getInfo: function(pid){
			$.get("../crm/feedback/info/"+pid, function(r){
	    ...
	    $("#jqGrid").jqGrid({
	        ...
	        colModel: [			
						{ label: 'pid', name: 'pid', index: 'pid', width: 50, key: true,hidden:true },
							...
							{ label: vm.$t('handle'), name: 'handle',  width: 80,formatter:function(value, options, row){							
								return  '<a onclick="vm.handle(\''+row.pid+'\')" href="javascript:">'+vm.$t('handle')+'</a>';
							}}								
			        ],
			viewrecords: true,
